...
/**
 * Filter that catches session repository errors at the lowest level This prevents
 * IllegalStateException from session invalidation from bubbling up and causing HTTP 500 errors or
 * error page cascades
 */
...
public class SessionRepositoryErrorFilter extends OncePerRequestFilter {
  ...
  private void handleSessionError(
    ...
    try {
      ...
      response.getWriter();
      logger.debug(
          "Response writer already accessed for path: {} - allowing to complete normally", path);
      return;
      ...
      if (writerException.getMessage() != null
        ...
        logger.debug(
            "Response output stream already accessed for path: {} - allowing to complete normally",
            path);
        return;
      }
    }
    ...
    if (path.startsWith("/error")) {
      ...
      response
          .getWriter()
          .write("<html><body>Session expired. Please refresh the page.</body></html>");
      return;
    }
