...
/**
 * Global exception handler specifically for session-related errors
 * This catches session invalidation errors that bubble up from the session repository
 */
@ControllerAdvice
...
public class GlobalSessionExceptionHandler {
  ...
  public ResponseEntity<Object> handleSessionInvalidationError(
      IllegalStateException e, WebRequest request, HttpServletRequest httpRequest, HttpServletResponse httpResponse) {
    
    if (e.getMessage() != null && e.getMessage().contains("Session was invalidated")) {
      String path = httpRequest.getRequestURI();
      logger.debug("Global session invalidation error handled for path: {} - {}", path, e.getMessage());
      
      // For API endpoints, return success since the business operation likely succeeded
      if (path.startsWith("/api/")) {
        logger.debug("Session invalidation on API endpoint - returning success response for path: {}", path);
        ...
        return ResponseEntity.ok()
            ...
            .body("{\"success\":true,\"warning\":\"Session cleanup issue - please refresh if you experience problems\"}");
      }
      ...
      logger.debug("Session invalidation on non-API endpoint - suggesting redirect for path: {}", path);
  ...
  public ResponseEntity<Object> handleGenericSessionError(
      Exception e, WebRequest request, HttpServletRequest httpRequest, HttpServletResponse httpResponse) {
    ...
    if (isSessionRelatedError(e)) {
      ...
      if (path.startsWith("/api/")) {
        ...
        return ResponseEntity.ok()
            ...
            .body("{\"success\":true,\"warning\":\"Session issue resolved - please refresh if you experience problems\"}");
      }
