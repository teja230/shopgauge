...
/**
 * Special filter to handle OAuth callback sessions more gracefully This prevents session
 * invalidation errors during the OAuth flow
 */
public class OAuthSessionFilter extends OncePerRequestFilter {
  ...
  protected void doFilterInternal(
    ...
    boolean isOAuthFlow =
        path.contains("/api/auth/shopify/callback")
    ...
    if (isOAuthFlow) {
      ...
      try {
        ...
        if (e.getMessage() != null && e.getMessage().contains("Session was invalidated")) {
          logger.warn(
              "OAuth session invalidation handled gracefully for path: {} - {}",
              path,
              e.getMessage());
          ...
          if (response.isCommitted()) {
            logger.info(
                "Response already committed for OAuth flow - allowing to complete normally");
            return;
          }
          ...
          if (path.contains("/callback")) {
            logger.warn(
                "Session invalidation during OAuth callback - this may indicate a configuration issue");
            // Let the callback handle this error appropriately
            throw e;
          } else {
            // For post-OAuth API calls, return success since the business operation likely
            // succeeded
            logger.info(
                "Session invalidation during post-OAuth API call - allowing success response");
            return;
          }
        } else {
          throw e;
        ...
        if (e.getMessage() != null && e.getMessage().contains("Session")) {
          logger.warn(
              "Session-related error during OAuth flow for path: {} - {}", path, e.getMessage());
