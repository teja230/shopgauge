...
public class PriceChangeCalculationService {
  ...
  public Optional<BigDecimal> calculatePriceChangePercent(Long competitorId, BigDecimal newPrice) {
    try {
      if (newPrice == null || newPrice.compareTo(BigDecimal.ZERO) <= 0) {
        logger.debug(
            "calculatePriceChangePercent: Invalid new price for competitor {}", competitorId);
      ...
      if (previousPrice.compareTo(BigDecimal.ZERO) <= 0) {
        logger.debug(
            "calculatePriceChangePercent: Previous price is zero or negative for competitor {}",
            competitorId);
        return Optional.empty();
      }
      ...
      logger.debug(
          "calculatePriceChangePercent: Competitor {} price change: ${} -> ${} = {}%",
      ...
      logger.warn(
          "calculatePriceChangePercent: Error calculating price change for competitor {}: {}",
          competitorId,
          e.getMessage());
      return Optional.empty();
    }
  ...
  private Optional<BigDecimal> getLastValidPrice(
    ...
    try {
      ...
      logger.debug(
          "getLastValidPrice: No valid price found for competitor {}: {}",
          competitorId,
          e.getMessage());
      return Optional.empty();
    }
  ...
  public Optional<BigDecimal> calculatePriceChangeOverPeriod(Long competitorId, int days) {
    try {
      ...
      List<Map<String, Object>> results =
          jdbcTemplate.queryForList(query, competitorId, competitorId, days);

      if (results.isEmpty()) {
        logger.debug(
            "calculatePriceChangeOverPeriod: No historical data found for competitor {} over {} days",
            competitorId,
            days);
        return Optional.empty();
      }
      ...
      logger.debug(
          "calculatePriceChangeOverPeriod: Competitor {} {}-day change: ${} -> ${} = {}%",
      ...
      logger.warn(
          "calculatePriceChangeOverPeriod: Error calculating {}-day price change for competitor {}: {}",
          days,
          competitorId,
          e.getMessage());
      return Optional.empty();
  ...
  public Map<String, Object> getPriceChangeStatistics(Long competitorId) {
    try {
      ...
      logger.error(
          "getPriceChangeStatistics: Error getting statistics for competitor {}: {}",
          competitorId,
          e.getMessage());
      return Map.of();
    }
  ...
  public void validateAndFixPriceChanges(Long competitorId) {
    try {
      logger.info(
          "validateAndFixPriceChanges: Starting validation for competitor {}", competitorId);
      ...
      if (snapshots.size() < 2) {
        logger.debug(
            "validateAndFixPriceChanges: Not enough snapshots for competitor {}", competitorId);
        return;
      }
      ...
      for (int i = 1; i < snapshots.size(); i++) {
        ...
        if (currentPrice != null
          ...
          if (storedChange == null || storedChange.compareTo(correctChange) != 0) {
            ...
            fixedCount++;
            logger.debug(
                "validateAndFixPriceChanges: Fixed price change for snapshot {}: {}% -> {}%",
      ...
      if (fixedCount > 0) {
        logger.info(
            "validateAndFixPriceChanges: Fixed {} price change calculations for competitor {}",
            fixedCount,
            competitorId);
      } else {
      ...
      logger.error(
          "validateAndFixPriceChanges: Error validating price changes for competitor {}: {}",
          competitorId,
          e.getMessage());
    }
  }
  ...
  public String getPriceTrend(Long competitorId, int days) {
    try {
      ...
      logger.warn(
          "getPriceTrend: Error analyzing price trend for competitor {}: {}",
          competitorId,
          e.getMessage());
      return "unknown";
    }
