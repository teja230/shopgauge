...
/**
 * Smart snapshot creation service that only creates snapshots on significant price changes
 * Reduces storage while preserving important price history
 */
@Service
public class SmartSnapshotService {
    ...
    public boolean shouldCreateSnapshot(Long competitorId, BigDecimal newPrice, boolean inStock) {
        try {
            ...
            if (!lastPrice.isPresent()) {
                logger.debug("No previous price found for competitor {}, creating first snapshot", competitorId);
                return true; // Always create first snapshot
            }

            // Check if price changed significantly
            if (isSignificantPriceChange(lastPrice.get(), newPrice)) {
                logger.info("Significant price change detected for competitor {}: ${} -> ${}", 
                    competitorId, lastPrice.get(), newPrice);
                return true;
            }
            ...
            if (hasMinimumIntervalPassed(competitorId)) {
                logger.debug("Minimum interval passed for competitor {}, creating regular snapshot", competitorId);
                return true;
            }
            ...
            logger.warn("Error determining snapshot creation for competitor {}: {}", competitorId, e.getMessage());
    ...
    private boolean isSignificantPriceChange(BigDecimal oldPrice, BigDecimal newPrice) {
        ...
        double changePercent = Math.abs(
            (newPrice.doubleValue() - oldPrice.doubleValue()) / oldPrice.doubleValue() * 100
        );
    ...
    private boolean hasMinimumIntervalPassed(Long competitorId) {
        try {
            ...
            java.sql.Timestamp lastChecked = jdbcTemplate.queryForObject(query, java.sql.Timestamp.class, competitorId);
            if (lastChecked == null) {
                return true; // No previous snapshot, allow creation
            }

            long hoursSinceLastCheck = (System.currentTimeMillis() - lastChecked.getTime()) / (1000 * 60 * 60);
    ...
    public java.util.List<Map<String, Object>> getPriceHistory(Long competitorId, int days) {
        try {
            ...
            logger.error("Error getting price history for competitor {}: {}", competitorId, e.getMessage());
    ...
    public Map<String, Object> getPriceStatistics(Long competitorId) {
        try {
            ...
            logger.error("Error getting price statistics for competitor {}: {}", competitorId, e.getMessage());
            return Map.of();
        }
    }
