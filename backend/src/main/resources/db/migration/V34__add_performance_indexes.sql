-- V34: Add Performance Indexes for Enhanced Columns
-- This migration adds indexes that depend on columns added in V32
...
ON market_intelligence_costs (cache_hit_rate DESC) 
...
ON price_alerts (change_percent DESC) 
...
-- Additional indexes that were in V30 but don't depend on V32 columns
...
-- Additional indexes for price_snapshots with significant changes
...
-- Additional indexes for price_snapshots summary queries
CREATE INDEX IF NOT EXISTS idx_price_snapshots_summary 
ON price_snapshots (competitor_url_id, checked_at DESC) 
INCLUDE (price, price_change_percent, in_stock);

-- Additional indexes for market_intelligence_costs monitoring
